<template>
  <div class="container">
    <div class="top-container">
      <div class="content-title">修改模块</div>
      <hr>
    </div>
        ...
        <el-scrollbar style="height:100%">
          <el-form ref="ModuleMgForm" :model="ModuleMgForm" label-width="100px" class="top-form">
            <el-form-item label="父模块:">
              <el-input ref="parentName" v-model="ModuleMgForm.parentId" />
...
<script>
import { saveMoudle, ModuleSelect, getAllMenu } from '@/api/addOrSave' // 保存修改接口
  ...
  data() {
    return {
      ...
      ModuleMgForm: {
        ...
        authUrl: this.$route.query.authUrl,
        attached: this.$route.query.attached,
  ...
  methods: {
    ...
    paIdToName() {
      getAllMenu().then(response => {
        ...
        for (const i in list) {
          if (this.ModuleMgForm.parentId == list[i].id) {
            this.ModuleMgForm.parentId = list[i].name
          }
        }
      })
    },
